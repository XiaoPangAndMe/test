...
- (void)setSelectionIndicator:(UIView *)selectionIndicator
{
...
- (void)setUp
{
    ...
    _scrollView.delaysContentTouches = _delaysContentTouches; //属性方法详见：http://www.cnblogs.com/wayne23/p/4011266.html
    
    _scrollView.bounces = _bounces && !_wrapEnabled; //反弹效果是否关闭
...
- (void)scrollToItemAtIndex:(NSInteger)index duration:(NSTimeInterval)duration
{
   ...
   /****************/
    ...
    if ([self.itemViews[@(index)] isMemberOfClass:[UILabel class]]) {
    
      NSArray *allKeys = [self.itemViews allKeys];
        for (NSNumber *index in allKeys) {
            UILabel *label = self.itemViews[index];
     ...
     CGSize size = [label.text sizeWithAttributes:[NSDictionary dictionaryWithObjectsAndKeys:label.font,NSFontAttributeName, nil]];
        ...
        self.selectionIndicator.frame = CGRectMake([UIScreen mainScreen].bounds.size.width/2.0 - size.width/2.0, self.bounds.size.height - 1.5,size.width , 1.5);
        }
   /****************/
